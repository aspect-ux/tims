...
public class AdminController {
    ...
    @Autowired
    private Sub_TeaService sub_teaService;
...
*/
        ...
        //change to subject management

        PageHelper.startPage(pn, 6);
        List<Sub_Tea> subjects = sub_teaService.getAllSubjects();
        PageInfo<Sub_Tea> page = new PageInfo<Sub_Tea>(subjects, 5);
        model.addAttribute("pageInfo",page);
        return "adm/classlist";
    ...
    //处理班级添加事务---更改为课程添加（排课系统）为课程分配老师
    @PostMapping(value = "/adm/classAdd")
    public String classAdd(@Valid Sub_Tea classes, BindingResult bindingResult,Model model)
    {
        List<ObjectError> allErrors = bindingResult.getAllErrors();
        List<MyError> errmsg = new ArrayList<>();
        if(allErrors.size()==0)
        {
            if(sub_teaService.selectByName(classes.getsubjectName())==null) {
                Classes classVail = classService.selectById(classes.getsubjectId());
                if (classVail == null) {
                    sub_teaService.addSubject(classes);
    ...
    //返回班级修改页面
    /* 将班级改为课程来使用 用与对表subject_teacher进行操作,如需使用班级的数据...
    @GetMapping(value = "/adm/class/{classId}")
    public String updateClassPage(@PathVariable("classId") String classId,Model model)
    {
        Classes classes=sub_teaService.selectById(classId);
        model.addAttribute("class",classes);
        return "adm/upadteclass";
    }*/
    @GetMapping(value = "/adm/class/{subId}")
    public String updateClassPage(@PathVariable("subId") String classId,Model model)
    {
        Sub_Tea classes=sub_teaService.selectById(classId);
        model.addAttribute("class",classes);
        return "adm/upadteclass";
    }
    ...
    public String updateClass(@Valid Sub_Tea classes,BindingResult bindingResult,Model model)
    {
        List<ObjectError> allErrors = bindingResult.getAllErrors();
        List<MyError> errmsg = new ArrayList<>();
        if(allErrors.size()==0)
        {
            ...
            sub_teaService.deleteById(classes.getsubjectId());
            sub_teaService.addSubject(classes);
            return "redirect:/adm/toclassdmin/1";
        }
        else
    ...
    {
        Sub_Tea classes=sub_teaService.selectByName(className);
        model.addAttribute("class",classes);
        return "adm/classbyid";
    }

    //处理删除班级事务
    @DeleteMapping(value = "/adm/class/{subId}")
    public String deleClass(@PathVariable("subId") String classId)
    {
        sub_teaService.deleteById(classId);
        return "redirect:/adm/toclassdmin/1";
    }
